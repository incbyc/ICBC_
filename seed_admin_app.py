...
def render_edit_existing_panel(
    ...
    if config.key == "icbc_sites":
        ...
        if lat_ok and lng_ok:
            st.info(
                "Site updated. Refresh the map page to see the marker "
...
def render_add_new_panel(
    ...
    if config.key == "icbc_sites":
        ...
        if lat_ok and lng_ok:
            st.info(
                "Site saved. Refresh the map page to see the marker — new and updated sites "
                "are loaded from Supabase (requires **Sync to Supabase on save**)."
            )
        else:
            st.warning("Add latitude and longitude so the site appears on the map.")
    if config.key == "staff":
...
def sidebar_settings():
    ...
    if sync_enabled and not sync_client:
